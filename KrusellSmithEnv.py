...
from FunctionApprox import LinearApprox
...
class KrusellSmithEnv:
    ...
    def __init__(self, params=None):
        ...
        self.duration_unemp_good = params.get(
            "duration_unemp_good",
            KrusellSmithEnv._default_params["duration_unemp_good"],
        )
        self.duration_unemp_bad = params.get(
            "duration_unemp_bad", KrusellSmithEnv._default_params["duration_unemp_bad"]
        )
        ...
        self.duration_good = params.get(
            "duration_good", KrusellSmithEnv._default_params["duration_good"]
        )
        self.duration_bad = params.get(
            "duration_bad", KrusellSmithEnv._default_params["duration_bad"]
        )
        ...
        self.transfer = params.get(
            "transfer", KrusellSmithEnv._default_params["transfer"]
        )
    ...
    def utility_func(self, c):
        return (
            c ** (1 - self.sigma) / (1 - self.sigma) if self.sigma != 1 else np.log(c)
        )

    def utility(self, c):
        return np.piecewise(
            c, [c <= 0, c > 0], [lambda x: 1e10 * (x - 1), self.utility_func]
        )
    ...
    def gen_transmat(self):
        ...
        # Pr{e'|e, z, z'}   z, z', e, e'
        self.e_trans_mat = np.array(
            [
                [[[pgg11, pgg01], [pgg10, pgg00]], [[pbg11, pbg01], [pbg10, pbg00]]],
                [[[pgb11, pgb01], [pgb10, pgb00]], [[pbb11, pbb01], [pbb10, pbb00]]],
            ]
        )
        ...
        self.logger.debug(
            "e Transition Matrix\nGood to Good:\n{}\nGood to Bad:\n{}\nBad to Good:\n{}\nBad to Bad:\n{}".format(
                self.e_trans_mat[0][0],
                self.e_trans_mat[0][1],
                self.e_trans_mat[1][0],
                self.e_trans_mat[1][1],
            )
        )

    def no_agg_risk_VFI(
        self, r, w, trans_mat, k_max=100, k_num=500, tol=1e-5, **optimize_params
    ):
        ...
        cash_in_hand = (
            (1 + r - self.delta) * k_grids.reshape((-1, 1))
            + self.h * w * e_grids.reshape((1, -1))
            + self.transfer
        )
        c_mat = (
            np.kron(cash_in_hand, np.ones(k_num)).reshape((k_num, e_num, k_num))
            - k_grids
        )
            ...
            if hpi and (iter % (hpi_value_iter + hpi_policy_iter)) >= hpi_value_iter:
                for idxk, k in enumerate(k_grids):
                    for idxe, e in enumerate(e_grids):
                        value_mat[idxk][idxe] = u_mat[idxk][idxe][
                            policy_mat[idxk][idxe]
                        ] + self.beta * np.dot(
                            trans_mat[idxe],
                            old_value_mat.transpose()[:, policy_mat[idxk][idxe]],
                        )
            else:
                value_mat = np.max(u_mat + self.beta * future_value.transpose(), axis=2)
                policy_mat = np.argmax(
                    u_mat + self.beta * future_value.transpose(), axis=2
                )
    ...
    def no_agg_risk_aggregation(self, policy_mat, k_grids, trans_mat, tol=1e-5):
        ...
        for idxk in range(len(k_grids)):
            for idxe in range(2):
                transition[idxk * 2 + idxe][policy_mat[idxk][idxe] * 2 + 0] = trans_mat[
                    idxe
                ][0]
                transition[idxk * 2 + idxe][policy_mat[idxk][idxe] * 2 + 1] = trans_mat[
                    idxe
                ][1]
    ...
    def no_agg_risk_equilibrium(
        self, state, r_init=0.03, max_iter=200, tol=1e-5, lr=0.5
    ):
        self.logger.info(
            "Calculating the stationary equilibrium without aggregate uncertainty when state is {}.".format(
                state
            )
        )
        if state == "good":
            trans_mat = np.array(
                [
                    [self.trans_mat[0][0], self.trans_mat[0][2]],
            ...
            trans_mat = np.array(
                [
                    [self.trans_mat[1][1], self.trans_mat[1][3]],
        ...
        for iter in range(max_iter):
            ...
            value_mat, policy_mat, k_grids = self.no_agg_risk_VFI(
                r, w, trans_mat, **optimize_dict
            )
            dist, k_supply = self.no_agg_risk_aggregation(
                policy_mat, k_grids, trans_mat
            )
            r_supply = self.alpha * z * (agg_l / k_supply) ** (1 - self.alpha)
            self.logger.debug(
                "Iteration {}: r demand {}, r supply {}, k demand {}, k supply {}, labor {}, wage {}.".format(
                    iter + 1, r, r_supply, agg_k, k_supply, agg_l, w
                )
            )
            if np.abs(agg_k - k_supply) < tol or (
                np.min(large_r) - np.max(small_r) < tol
            ):
                solved = True
                self.logger.info(
                    "Solution: interest rate {}, capital {}, labor {}, wage {}.".format(
                        r, agg_k, agg_l, w
                    )
                )
                break
        ...
        if not solved:
            self.logger.warning(
                "Cannot solve the stationary equilibrium without aggregate uncertainty when state is {}.".format(
                    state
                )
            )
        ...
        for iter in range(max_iter):
            ...
            for idxz, z in enumerate(z_grids):
                for idx_aggk, agg_k in enumerate(agg_k_grids):
                    ...
                    # right hand side
                    cash_in_hand = (
                        w * self.h * e_grids.reshape((1, -1))
                        + (1 - self.delta + r) * k_grids.reshape((-1, 1))
                        + self.transfer
                    )
                    c_mat = (
                        np.kron(cash_in_hand, np.ones(k_num)).reshape(
                            (k_num, e_num, k_num)
                        )
                        - k_grids
                    )
                    ...
                    for idxe, idxz_p, idxk in product(range(2), range(2), range(k_num)):
                        coeff = np.polyfit(
                            agg_k_grids,
                            old_value_mat[:, idxe, idxz_p, idxk],
                            len(agg_k_grids) - 1,
                        )
                        local_value_mat[idxe][idxz_p][idxk] = np.poly1d(coeff)(
                            agg_k_prime
                        )
                    ...
                    # on the grid policy
                    policy_mat[idx_aggk, :, idxz, :] = k_grids[
                        np.argmax(rhs, axis=2).transpose()
                    ]
            ...
            if loss < tol and (
                not hpi
                or (iter + 1) % (hpi_value_iter + hpi_policy_iter) < hpi_value_iter
            ):
                break
    ...
    def refine_policy(
        self, k_motion, value_mat, k_grids, agg_k_grids, k_num=200, agg_k_num=50
    ):
        ...
        for idxz, z in enumerate(z_grids):
            for idx_aggk, agg_k in enumerate(agg_k_grids):
                ...
                for idxe, idxz_p, idxk in product(range(2), range(2), range(old_k_num)):
                    coeff = np.polyfit(
                        old_aggk_grids,
                        value_mat[:, idxe, idxz_p, idxk],
                        len(old_aggk_grids) - 1,
                    )
    ...
    def simulate_aggk(
        ...
        if z_series is None:
            idxz_series, z_series = FiniteMarkov(
                np.array([self.zg, self.zb]), self.z_trans_mat
            ).simulate(length, self.zg)
        else:
        ...
        policy_func = [
            [
                interp2d(agg_k_grids, k_grids, policy_mat[:, 0, 0, :].transpose()),
                interp2d(agg_k_grids, k_grids, policy_mat[:, 0, 1, :].transpose()),
            ],
            [
                interp2d(agg_k_grids, k_grids, policy_mat[:, 1, 0, :].transpose()),
                interp2d(agg_k_grids, k_grids, policy_mat[:, 1, 1, :].transpose()),
            ],
        ]  # e, z

        # initialize
        kvec = np.random.random(agent_num) * (
            np.max(agg_k_grids) - np.min(agg_k_grids)
        ) + np.min(agg_k_grids)
        evec = np.zeros(agent_num, dtype=int)
        evec[np.random.random(agent_num) < self.u_good] = (
            1  # 1 = unemployed, 0 = employed, state index
        )
        agg_k = np.zeros(length)
        for t in range(length):
            agg_k[t] = np.mean(kvec)
            kvec = (1 - evec) * policy_func[0][idxz_series[t]](
                agg_k[t], kvec
            ).ravel() + evec * policy_func[1][idxz_series[t]](agg_k[t], kvec).ravel()
    ...
    def solve_approximated_equilibrium(
        self,
        order=1,
        max_iter=200,
        ...
        for iter in range(max_iter):
            ...
            value_mat, policy_mat, k_grids, agg_k_grids = self.KS_vfi(
                k_motion, value_init=value_mat, tol=vfi_tol, **optimize_dict
            )
            ...
            agg_k, idxz, z = self.simulate_aggk(
                policy_mat,
                k_grids,
                agg_k_grids,
            ...
            for idxz_value in [0, 1]:
                ...
                plt.plot(
                    np.exp(local_x),
                    np.exp(k_motion[idxz_value].eval(local_x.reshape(-1, 1))),
                    label=KrusellSmithEnv.z_name[idxz_value],
                )
                self.logger.info(
                    "Regression for {} state: log(K') = {} + {} * log(K), r2 = {}.".format(
                        KrusellSmithEnv.z_name[idxz_value],
                        round(weights[0], 3),
                        round(weights[1], 3),
                        r2,
                    )
    ...
    def plot_policy(
        ...
        for idx_aggk in range(len(agg_k_grids)):
            for idxz in [0, 1]:
                for idxe in [0, 1]:
                    plt.plot(
                        k_grids[k_grids < max_k],
                        policy_mat[idx_aggk][idxe][idxz][k_grids < max_k],
                        label="idiosyncratic state {}".format(
                            KrusellSmithEnv.e_name[idxe]
                        ),
                    )
                plt.plot(
                    k_grids[k_grids < max_k],
                ...
                plt.legend()
                plt.title(
                    "Decision Rule: aggregate state {}, aggregate K = {}, ".format(
                        KrusellSmithEnv.z_name[idxz], round(agg_k_grids[idx_aggk], 2)
                    )
                )
                if figure_path is not None:
                    plt.savefig(
                        os.path.join(
                            figure_path, "policy_{}_{}.pdf".format(idx_aggk, idxz)
                        )
                    )
                    plt.savefig(
                        os.path.join(
                            figure_path, "policy_{}_{}.png".format(idx_aggk, idxz)
                        )
                    )
                plt.show()
    ...
    def full_simulation(self, z_series=None, length=11000, agent_num=5000):
        ...
        policy_func = [
            [
                interp2d(
                    self.agg_k_grids,
                    self.k_grids,
                    self.policy_mat[:, 0, 0, :].transpose(),
                ),
                interp2d(
                    self.agg_k_grids,
                    self.k_grids,
                    self.policy_mat[:, 0, 1, :].transpose(),
                ),
            ],
            [
                interp2d(
                    self.agg_k_grids,
                    self.k_grids,
                    self.policy_mat[:, 1, 0, :].transpose(),
                ),
                interp2d(
                    self.agg_k_grids,
                    self.k_grids,
                    self.policy_mat[:, 1, 1, :].transpose(),
                ),
            ],
        ]  # e, z
        ...
        kvec[0] = np.random.random(agent_num) * (
            np.max(self.k_grids) - np.min(self.k_grids)
        ) + np.min(self.k_grids)
        evec = np.zeros((length + 1, agent_num), dtype=int)
        evec[0][
            np.random.random(agent_num) < self.u_good
        ] = 1  # 1 = unemployed, 0 = employed, state index
        ...
        for t in range(length):
            ...
            kvec[t + 1] = (1 - evec[t]) * policy_func[0][idxz_series[t]](
                agg_k[t], kvec[t]
            ).ravel() + evec[t] * policy_func[1][idxz_series[t]](
                agg_k[t], kvec[t]
            ).ravel()
            prob = self.e_trans_mat[idxz_series[t], idxz_series[t + 1], :, 0]
            ran_vec = np.random.random(agent_num)
        ...
        agg_c = yvec[:-1] - ivec
        income_vec = (rvec - self.delta) * kvec.transpose() + wvec * self.h * (
            1 - evec.transpose()
        )  # N, T
    ...
    def forecast_aggk(self, k_init, idxz_series, periods=1):
        ...
        for t in range(periods):
            k = idxz_series[t] * self.k_motion[1].eval(k.reshape(-1, 1)) + (
                1 - idxz_series[t]
            ) * self.k_motion[0].eval(k.reshape(-1, 1))
        return np.exp(k)

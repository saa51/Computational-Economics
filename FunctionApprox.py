...
class ChebyshevApprox2D(FunctionApprox):
    def __init__(
        ...
        self.grid1 = (
            ...
            else np.cos(np.pi * np.arange(1, 2 * n1, 2) / (2 * n1))
        )
        self.grid2 = (
            (grid2 - mean2) / width2
            if grid2 is not None
            else np.cos(np.pi * np.arange(1, 2 * n2, 2) / (2 * n2))
        )
        value1 = np.cos(
            np.expand_dims(np.arccos(self.grid1), axis=-1) * np.arange(n1)
        ).reshape((n1, 1, n1, 1))
        value2 = np.cos(
            np.expand_dims(np.arccos(self.grid2), axis=-1) * np.arange(n2)
        ).reshape((1, n2, 1, n2))
    ...
    def real_grids(self) -> Tuple[np.ndarray, np.ndarray]:
        return (
            self.grid1 * self.width1 + self.mean1,
            self.grid2 * self.width2 + self.mean2,
        )
    ...
    def __call__(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        value1 = np.cos(
            np.expand_dims(np.arccos(x), axis=-1) * np.arange(self.n1)
        ).reshape(x.shape + (1, self.n1, 1))
        value2 = np.cos(
            np.expand_dims(np.arccos(y), axis=-1) * np.arange(self.n2)
        ).reshape((1,) + y.shape + (1, self.n2))
    ...
    def approx(
        self, values: np.ndarray, lr: float = 1, update: bool = True
    ) -> np.ndarray:
...
class PolynomialApprox(FunctionApprox):
    ...
    def approx(
        ...
        new_weights = (
            inv(basis_values.T @ basis_values) @ basis_values.T @ values.reshape((-1,))
        )
        new_weights = lr * new_weights + (1 - lr) * self.weights
        if update:
            self.weights = new_weights
    ...
    def gradient(self, x: np.ndarray) -> np.ndarray:
        x = np.array(x)
        ans = np.expand_dims(x, axis=-1) ** np.arange(self.deg - 1) @ (
            np.arange(1, self.deg) * self.weights[1:]
        )
        return ans
...
class SplineApprox(FunctionApprox):
    def __init__(self) -> None:
        self.grids: Optional[np.ndarray] = None
        self.values: Optional[np.ndarray] = None

    def __call__(self, x: Union[float, np.ndarray]) -> np.ndarray:
        idx_h = np.searchsorted(self.grids[:-1], x, "left")  # type: ignore
        idx_l = idx_h - 1
        return (self.values[idx_h] - self.values[idx_l]) / (  # type: ignore
            self.grids[idx_h] - self.grids[idx_l]
        ) * (x - self.grids[idx_l]) + self.values[
            idx_l
        ]  # type: ignore

    def approx(self, grids: np.ndarray, values: np.ndarray, lr: float = 1) -> None:
...
class ChebyshevApprox(FunctionApprox):
    def __init__(
        ...
        if grids is None:
            self.default_grids = (
                np.cos(np.pi * np.arange(1, 2 * n, 2) / (2 * n)) * width + mean
            )
        else:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        basis_values = np.cos(
            np.expand_dims(np.arccos(x_g), axis=-1) * np.arange(self.deg)
        )
    ...
    def approx(
        ...
        basis_values = np.cos(
            np.expand_dims(np.arccos(g_g), axis=-1) * np.arange(self.deg)
        )
    ...
    def gradient(self, x: np.ndarray) -> np.ndarray:
        x_g = (x - self.mean) / self.width
        basis_values = np.sin(
            np.expand_dims(np.arccos(x_g), axis=-1) * np.arange(self.deg - 1)
...
class LinearApprox(FunctionApprox):
    def __init__(self, var_num: int, order: int = 1, constant: bool = True) -> None:
        self.v_num = var_num
        self.order = order + 1
        self.cons = constant
        ...
        self.param_num += int(
            comb(np.arange(order) + self.v_num, (self.v_num - 1) * np.ones(order)).sum()
        )

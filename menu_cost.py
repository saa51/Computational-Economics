...
class MenuCostEnv:
    ...
    def approx_eta(
        self, eta_num: int, eta_precision: int
    ...
    def vfi(
        ...
        profit = np.exp(-self.theta * (policy_grid + self.mu)) * (
            np.exp(policy_grid + self.mu)
            - np.expand_dims(
                (self.theta - 1) / self.theta * np.exp(-a_grid), axis=-1
            ).repeat(policy_num, axis=-1)
        )
        ...
        for i in range(policy_num):
            if offset + i + eta_precision >= p_num:
                break
            pi[offset + i + eta_precision, :, i] += (
                (self.theta - 1) / self.theta * self.k
            )
        ...
        while not np.allclose(value, old_v):
            old_v = np.copy(value)
            value = np.max(
                pi + self.beta * trans_mat_a @ value.T @ trans_mat_p.T, axis=-1
            )
    ...
    def plot_policy(
        self,
        res: Optional[MenuCostVFIRes] = None,
        ...
        for idxa in range(res.value.shape[1]):
            plt.plot(
                res.p_grid,
                res.policy_grid[res.policy[:, idxa]] + self.mu,
                label=f"A={np.exp(res.a_grid[idxa]):.4f}",
            )
    ...
    def simulate(
        ...
        for t in range(1, T + 1):
            ...
            p_paths[:, t] = (
                2 * offset
                + res.policy[(p_paths[:, t - 1], a_paths[:, t - 1])]
                - eta_paths[:, t]
            )
        if annualize:
            return self.annualize(
                res.p_grid[p_paths][:, 1:] + cpi_paths[:, 1:],
                res.a_grid[a_paths][:, 1:],
                cpi_paths[:, 1:],
                res.eta_grid[eta_paths][:, 1:],
    ...
    def annualize(
        ...
        def annual_map(data: np.ndarray) -> np.ndarray:
            n, T = data.shape
    ...
    def hazard_function(
        ...
        stay_put = np.equal(
            (res.policy + res.precision + res.offset).T, np.arange(len(res.p_grid))
        ).T
        ...
        for i in range(len(res.p_grid)):
            if 0 <= i - res.offset and i + res.offset < len(res.p_grid):
                trans_p_stay[i, i - res.offset : i - res.offset + len(res.eta_grid)] = (
                    res.eta_prob
                )

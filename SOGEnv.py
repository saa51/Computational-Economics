...
class SOGVFIResult:
    ...
    def plot_capital_diff(self, title="", fname=None):
        for idxa in range(len(self.a_grid)):
            ...
            plt.plot(
                self.k_grid,
                self.k_grid[self.policy[:, idxa]] - self.k_grid,
                label="a=" + str(round(a, 2)),
            )
        plt.plot()
        plt.legend()
    ...
    def euler_error(self, grids):
        ...
        k_list = grids[:, 0]
        idxa_list = np.argmin(
            (grids[:, 1].reshape(-1, 1) - self.a_grid.reshape(1, -1)) ** 2, axis=1
        )
        res = []
        for k, idxa in zip(k_list, idxa_list):
            k_prime = policy_func[idxa].eval(k)
            c = (
                np.exp(self.a_grid[idxa]) * k**self.alpha
                + (1 - self.delta) * k
                - k_prime
            )
            k_2primes = np.array(
                [policy_func[i].eval(k_prime) for i in range(len(self.a_grid))]
            )
            c_primes = (
                np.exp(self.a_grid) * k_prime**self.alpha
                + (1 - self.delta) * k_prime
                - k_2primes
            )
            r_primes = (
                self.alpha * np.exp(self.a_grid) * k_prime ** (self.alpha - 1)
                + 1
                - self.delta
            )
            res.append(
                ...
                - (
                    self.beta
                    * np.sum(r_primes * self.a_trans[idxa] * c_primes ** (-self.gamma))
                )
                ** (-1 / self.gamma)
                / c
            )
    ...
    def plot_euler_err(self, title="", fname=None, show=False):
        ...
        for idxa in range(len(self.a_grid)):
            grids = [
                (self.k_grid[i], self.a_grid[idxa]) for i in range(len(self.k_grid))
            ]
    ...
    def plot_value_derivative(self, title="", fname=None, show=False):
        for idxa in range(len(self.a_grid)):
            a = self.a_grid[idxa]
            plt.plot(
                (self.k_grid[1:] + self.k_grid[:-1]) / 2,
                (self.value[1:, idxa] - self.value[:-1, idxa])
                / (self.k_grid[1:] - self.k_grid[:-1]),
                label="a=" + str(round(a, 2)),
            )
        plt.legend()
    ...
    def plot_value_2derivative(self, title="", fname=None, show=False):
        for idxa in range(len(self.a_grid)):
            a = self.a_grid[idxa]
            gradient = (self.value[1:, idxa] - self.value[:-1, idxa]) / (
                self.k_grid[1:] - self.k_grid[:-1]
            )
            gradient_2 = (
                (gradient[1:] - gradient[:-1])
                / (self.k_grid[2:] - self.k_grid[:-2])
                * 2
            )
            plt.plot(
                (self.k_grid[2:] + self.k_grid[:-2]) / 2,
                gradient_2,
                label="a=" + str(round(a, 2)),
            )
        plt.legend()
    ...
    def plot_policy_derivative(self, title="", fname=None, show=False):
        for idxa in range(len(self.a_grid)):
            a = self.a_grid[idxa]
            gradient = (
                self.k_grid[self.policy[1:, idxa]] - self.k_grid[self.policy[:-1, idxa]]
            ) / (self.k_grid[1:] - self.k_grid[:-1])
            plt.plot(
                (self.k_grid[1:] + self.k_grid[:-1]) / 2,
                gradient,
    ...
    def simulate(self, periods=2500, tfp_series=None):
        ...
        if tfp_series is None:
            idxa_series, a_series = FiniteMarkov(self.a_grid, self.a_trans).simulate(
                periods, 0
            )
...
class SOGENV:
    ...
    def __init__(self, normalized=True, **kwargs):
        ...
        if normalized:
            ...
            self.k_ss = ((1 / self.beta - 1 + self.delta) / self.alpha) ** (
                1 / (self.alpha - 1)
            )
    ...
    def utility(self, c):
        u = np.ones_like(c)
        u[c > 0] = (
            c ** (1 - self.gamma) / (1 - self.gamma) if self.gamma != 1 else np.log(c)
        )
        u[c <= 0] = -np.inf
        return u
    ...
    def value_func_iter(self, nk, na, width, m=3, **optimize_params):
        ...
        cash_on_hand = (
            np.kron(np.exp(a_grids), k_grids**self.alpha).reshape((na, nk))
            + (1 - self.delta) * k_grids
        )
        u_mat = self.utility(
            np.repeat(np.expand_dims(cash_on_hand.T, -1), nk, axis=-1) - k_grids
        )
        ...
        while not np.allclose(old_value_mat, value_mat, atol=self.tol, rtol=0):
            ...
            if hpi and iter % hpi_policy_iter == 0:
                local_u = np.take_along_axis(
                    u_mat, np.expand_dims(policy_mat, axis=-1), axis=-1
                ).squeeze(axis=-1)
                local_vmat = np.copy(value_mat)
                for _ in range(hpi_value_iter):
                    value_mat = np.broadcast_to(trans_mat @ local_vmat.T, (nk, na, nk))
                    value_mat = local_u + self.beta * np.take_along_axis(
                        value_mat, np.expand_dims(policy_mat, axis=-1), axis=-1
                    ).squeeze(axis=-1)
                    if mqp:
                ...
                value_mat = np.max(
                    u_mat + self.beta * trans_mat @ old_value_mat.T, axis=-1
                )
    ...
    def value_approx(self, nk, na, width, deg, m, **optimize_params):
        ...
        def local_solve(value_func, item):
            ...
            def opposite_value(k_prime):
                return -(
                    self.utility(
                        np.exp(a) * k**self.alpha + (1 - self.delta) * k - k_prime
                    )
                    + self.beta
                    * np.sum(
                        np.array([value_func[i].eval(k_prime) for i in range(na)])
                        * trans_mat[idxa]
                    )
                )
            ...
            res = minimize_scalar(
                opposite_value, bounds=(k_min, min(cash, k_max)), method="bounded"
            )
        ...
        while not np.allclose(old_value_mat, value_mat, atol=self.tol, rtol=0):
            ...
            if hpi and (iter % (hpi_value_iter + hpi_policy_iter)) >= hpi_value_iter:
                ...
                if "parallel" not in optimize_params.get("methods", []):
                    res = map(
                        partial_iter,
                        list(product(enumerate(k_grids), enumerate(a_grids))),
                    )
                else:
                    res = Parallel(n_jobs=n_jobs)(
                        delayed(partial_iter)(x)
                        for x in product(enumerate(k_grids), enumerate(a_grids))
                    )
    ...
    def modified_pea(self, nk, na, width, deg, m):
        ...
        for idxa in range(na):
            exp_func[idxa].approx(
                [self.c_ss ** (-self.gamma) for _ in self.k_grids], grids=k_grids
            )
        ...
        while not np.allclose(old_weights, new_weights, rtol=0, atol=self.tol):
            ...
            for idxa, a in enumerate(self.a_grids):
                y = (
                    np.exp(a) * self.k_grids**self.alpha
                    + (1 - self.delta) * self.k_grids
                )
                ...
                for idxa_p, a_prime in enumerate(self.a_grids):
                    r_prime = (
                        self.alpha * np.exp(a_prime) * k_prime ** (self.alpha - 1)
                        + 1
                        - self.delta
                    )
                    y_prime = (
                        np.exp(a_prime) * k_prime**self.alpha
                        + (1 - self.delta) * k_prime
                    )
    ...
    def simulate(self, periods=2500, tfp_series=None):
        ...
        if tfp_series is None:
            idxa_series, a_series = FiniteMarkov(self.a_grids, self.trans_mat).simulate(
                periods, 0
            )
            tfp_series = (idxa_series, a_series)
        else:
    ...
    def endo_grid(self, nk, na, width, deg, m, init=None):
        ...
        self.k_grids = k_grids
        future_vfunc = [
            ChebyshevApprox(self.k_ss, width + 1e-6, deg) for _ in range(na)
        ]

        for idxa, a in enumerate(self.a_grids):
            if init is None:
                future_vfunc[idxa].approx(
                    k_grids * self.c_ss ** (-self.gamma), grids=k_grids
                )
            else:
                v_mat, p_mat = init
                k_prime = p_mat[:, idxa]
                future_value = v_mat[:, idxa] - self.utility(
                    np.exp(a) * self.k_grids**self.alpha
                    + (1 - self.delta) * self.k_grids
                    - k_prime
                )
        ...
        while not np.allclose(old_weights, new_weights, rtol=0, atol=self.tol):
            ...
            for idxa, a in enumerate(self.a_grids):
                ...
                y = c + self.k_grids
                y = np.maximum(
                    y, np.exp(a) * k_min**self.alpha + (1 - self.delta) * k_min
                )
                y = np.minimum(
                    y, np.exp(a) * k_max**self.alpha + (1 - self.delta) * k_max
                )
                c = y - self.k_grids
            ...
            for idxk, k in enumerate(self.k_grids):
                for idxa, a in enumerate(self.a_grids):
                    value_mat[idxk][idxa] = self.beta * np.dot(
                        [
                            value_func[a_p].eval(
                                np.exp(self.a_grids[a_p]) * k**self.alpha
                                + (1 - self.delta) * k
                            )
                            for a_p in range(na)
                        ],
            ...
            for idxa in range(na):
                new_weights.append(
                    future_vfunc[idxa].approx(value_mat[:, idxa], self.k_grids)
                )
            new_weights = np.array(new_weights).ravel()
        ...
        for idxa, a in enumerate(self.a_grids):
            ...
            for y in y_vec:
                ...
                res = root_scalar(
                    surplus,
                    args=(a, self.alpha, self.delta, y),
                    x0=self.k_ss,
                    fprime=surplus_prime,
                    fprime2=surplus_2prime,
                    method="halley",
                )
    ...
    def euler_error(self, grids):
        ...
        k_list = grids[:, 0]
        idxa_list = np.argmin(
            (grids[:, 1].reshape(-1, 1) - self.a_grids.reshape(1, -1)) ** 2, axis=1
        )
        res = []
        for k, idxa in zip(k_list, idxa_list):
            k_prime = policy_func[idxa].eval(k)
            c = (
                np.exp(self.a_grids[idxa]) * k**self.alpha
                + (1 - self.delta) * k
                - k_prime
            )
            k_2primes = np.array([policy_func[i].eval(k_prime) for i in range(na)])
            c_primes = (
                np.exp(self.a_grids) * k_prime**self.alpha
                + (1 - self.delta) * k_prime
                - k_2primes
            )
            r_primes = (
                self.alpha * np.exp(self.a_grids) * k_prime ** (self.alpha - 1)
                + 1
                - self.delta
            )
            res.append(
                ...
                - (
                    self.beta
                    * np.sum(
                        r_primes * self.trans_mat[idxa] * c_primes ** (-self.gamma)
                    )
                )
                ** (-1 / self.gamma)
                / c
    ...
    def plot_value(self, title="", fname=None):
        ...
        for idxa in range(self.value_mat.shape[1]):
            a = self.a_grids[idxa]
            plt.plot(
                self.k_grids, self.value_mat[:, idxa], label="a=" + str(round(a, 2))
            )
        plt.legend()
        plt.title("Value Function: " + title)
        if fname is not None:
    ...
    def plot_policy(self, title="", fname=None):
        ...
        for idxa in range(self.policy_mat.shape[1]):
            a = self.a_grids[idxa]
            plt.plot(
                self.k_grids, self.policy_mat[:, idxa], label="a=" + str(round(a, 2))
            )
    ...
    def plot_capital_diff(self, title="", fname=None):
        ...
        for idxa in range(self.policy_mat.shape[1]):
            ...
            plt.plot(
                self.k_grids,
                self.policy_mat[:, idxa] - self.k_grids,
                label="a=" + str(round(a, 2)),
            )
        plt.plot()
        plt.legend()
    ...
    def plot_euler_err(self, title="", fname=None):
        ...
        for idxa in range(na):
            grids = [
                (self.k_grids[i], self.a_grids[idxa]) for i in range(self.k_grids.size)
            ]
            err = self.euler_error(grids)
            err = np.log10(np.abs(err))
    ...
    def plot_value_derivative(self, title="", fname=None):
        ...
        for idxa in range(self.value_mat.shape[1]):
            ...
            plt.plot(
                self.k_grids[:-1],
                (self.value_mat[1:, idxa] - self.value_mat[:-1, idxa])
                / (self.k_grids[1] - self.k_grids[0]),
                label="a=" + str(round(a, 2)),
            )
        plt.legend()
        plt.title("Value Function Derivative: " + title)
    ...
    def plot_value_2derivative(self, title="", fname=None):
        ...
        for idxa in range(self.value_mat.shape[1]):
            a = self.a_grids[idxa]
            gradient = (self.value_mat[1:, idxa] - self.value_mat[:-1, idxa]) / (
                self.k_grids[1] - self.k_grids[0]
            )
            plt.plot(
                self.k_grids[:-2],
                (gradient[1:] - gradient[:-1]) / (self.k_grids[1] - self.k_grids[0]),
                label="a=" + str(round(a, 2)),
            )
        plt.legend()
        plt.title("Value Function 2nd Derivative: " + title)
    ...
    def plot_policy_derivative(self, title="", fname=None):
        ...
        for idxa in range(self.policy_mat.shape[1]):
            ...
            plt.plot(
                self.k_grids[:-1],
                (self.policy_mat[1:, idxa] - self.policy_mat[:-1, idxa])
                / (self.k_grids[1] - self.k_grids[0]),
                label="a=" + str(round(a, 2)),
            )
        plt.legend()
    ...
    def plot_policy_2derivative(self, title="", fname=None):
        ...
        for idxa in range(self.policy_mat.shape[1]):
            a = self.a_grids[idxa]
            gradient = (self.policy_mat[1:, idxa] - self.policy_mat[:-1, idxa]) / (
                self.k_grids[1] - self.k_grids[0]
            )
            plt.plot(
                self.k_grids[:-2],

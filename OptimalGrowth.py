...
def compute_moments(
    ...
    rho_y = np.mean(y[burned:-1] * y[burned + 1 :]) / np.mean(
        y[burned:-1] * y[burned:-1]
    )
...
class OptimalGrowthEnv:
    def __init__(self, params: Optional[Dict[str, float]] = None) -> None:
        ...
        self.value_func: Optional[np.ndarray] = None
        self.policy_func: Optional[np.ndarray] = None
        self.k_grids: Optional[np.ndarray] = None
        self.a_grids: Optional[np.ndarray] = None
        self.trans_mat: Optional[np.ndarray] = None
        self.policy_func_cheby: Optional[ChebyshevApprox2D] = None
        self.value_func_cheby: Optional[ChebyshevApprox2D] = None

    def k_new(self, k_old: float, a: float, c: float) -> float:
        return np.exp(a) * k_old**self.alpha + (1 - self.delta) * k_old - c

    def utility(self, c: np.ndarray) -> np.ndarray:
        u = np.zeros_like(c)
        u[c > 0] = (
            c[c > 0] ** (1 - self.gamma) / (1 - self.gamma)
            if self.gamma != 1
            else np.log(c[c > 0])
        )
        u[c <= 0] = -np.inf
    ...
    def resource(self, k: float, a: float) -> float:
    ...
    def grid_search(
        ...
        k_prime_grids = np.arange(
            self.k_ss - k_width, self.k_ss + k_width, c_grids_width
        )

        value_func = ChebyshevApprox2D(
            n_k, n_a, self.k_ss, a_center, k_width, a_width, grid2=a_grids
        )
        k_grids, a_grids = value_func.real_grids()
        resource = (
            np.exp(a_grids).reshape((-1, 1)) * k_grids.reshape((1, -1)) ** self.alpha
        ...
        while not np.allclose(
            value_func.weights, old_weights, atol=self.tol, rtol=self.tol
        ):
            ...
            new_values = np.take_along_axis(
                new_values, np.expand_dims(max_idx, axis=-1), axis=-1
            )
            policy_func = k_prime_grids[max_idx]
            value_func.approx(new_values)

            t.set_description(
                f"grid search: {np.sum(np.abs(value_func.weights - old_weights))}"
            )
            t.update()
    ...
    def euler_method(self, n_k, k_width, n_a, markov_method="R"):
        ...
        policy_func = ChebyshevApprox2D(
            n_k, n_a, self.k_ss, a_center, k_width, a_width, grid1=None, grid2=a_grids
        )
        value_func = ChebyshevApprox2D(
            n_k, n_a, self.k_ss, a_center, k_width, a_width, grid1=None, grid2=a_grids
        )
        ...
        while not np.allclose(policy_func.weights, old_weights, atol=self.tol):
            ...
            t.set_description(
                f"policy loss: {np.sum(np.abs(policy_func.weights - old_weights))}"
            )
            t.update()
            ...
            resource_ = (
                k_prime.reshape((n_k, n_a, 1)) ** self.alpha
                @ np.exp(a_grids).reshape((1, -1))
            ).transpose(2, 0, 1) + (1 - self.delta) * k_prime
            resource_ = resource_.transpose(1, 2, 0)
            c_prime = resource_ - k_2prime
            mu_prime = c_prime ** (-self.gamma) * (
                self.alpha
                * np.expand_dims(k_prime, axis=-1) ** (self.alpha - 1)
                * np.exp(a_grids)
                + 1
                - self.delta
            )
        ...
        while not np.allclose(
            ...
            t.set_description(
                f"value loss: {np.sum(np.abs(value_func.weights - old_weights))}"
            )
            t.update()
            old_weights = np.copy(value_func.weights)
            v_prime = value_func(k_prime, a_grids)
            new_values = u + self.beta * (v_prime * trans_mat).sum(axis=-1)
            value_func.approx(new_values, lr=1.0)
        self.value_func = new_values
    ...
    def plot_policy(self, grid_num=100, offgrid=True):
        ...
        k_grids = (
            np.linspace(
                np.min(self.k_grids), np.max(self.k_grids), grid_num, endpoint=True
            )
            if offgrid
            else self.k_grids
        )
        k_prime = (
            self.policy_func_cheby(k_grids, self.a_grids)
            if self.policy_func_cheby
            else self.policy_func
        )
    ...
    def plot_value(self, grid_num=100, offgrid=True):
        ...
        value = (
            self.value_func_cheby(k_grids, self.a_grids)
            if self.value_func_cheby
            else self.value_func
        )
        for idxa, a in enumerate(self.a_grids):
    ...
    def plot_consumption(self, grid_num=100, offgrid=True):
        ...
        resource = (
            np.expand_dims(np.exp(self.a_grids), axis=-1) * k_grids**self.alpha
            + (1 - self.delta) * k_grids
        )
        c = resource.T - k_prime
        resource = (
            np.expand_dims(np.exp(self.a_grids), axis=-1) * self.k_grids**self.alpha
            ...
            for idxk in range(k_num):
                for idxa in range(a_num):
                    for idxap in range(a_num):
                        distribution[policy_func[idxk][idxa]][idxap] += (
                            old_distribution[idxk][idxa] * self.trans_mat[idxa][idxap]
                        )
    ...
    def compute_moments(self, grid_num=100):
        ...
        rho_hat = np.sum(a_aprime * (self.trans_mat * dist_a)) / np.sum(
            self.a_grids * self.a_grids * dist_a
        )

        epsilon = -rho_hat * self.a_grids.reshape((-1, 1)) + self.a_grids.reshape(
            (1, -1)
        )
        ...
        sigma_ka_hat = (
            np.sum((k_values.reshape((-1, 1)) * self.a_grids.reshape((1, -1))) * dist)
            - mean_a_hat * mean_k_hat
        )
    ...
    def simulation(self, periods=5010000, a_series=None):
        if a_series is None:
            a_series = AR1Process(
                rho=self.rho, err_params={"sigma": self.sigma}
            ).simulate(periods, init=0)

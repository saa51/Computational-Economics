...
class MarkovProcess:
    def simulate(self, length: int, init: float) -> np.ndarray:
        pass
...
class AR1Process(MarkovProcess):
    def __init__(
        ...
        err_params: Optional[Dict[str, Any]] = None,
    ) -> None:
        self.rho = rho
        self.err = error_type
    ...
    def simulate(self, length: int, init: float) -> np.ndarray:
        ...
        for t in range(1, length + 1):
            simulation[t] = simulation[t - 1] * self.rho + np.random.normal(
                0, self.err_params["sigma"]
            )
        return simulation


class FiniteMarkov(MarkovProcess):
    def __init__(self, grids: np.ndarray, trans_mat: np.ndarray) -> None:
        self.grids = grids
        self.trans_mat = trans_mat

    def simulate(self, length: int, init: float) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for t in range(1, length + 1):
            simulation_idx[t] = np.searchsorted(
                trans_cdf[simulation_idx[t - 1]], rands[t]
            )
...
if __name__ == "__main__":
    mc = FiniteMarkov(
        np.array([-1, 0, 1]),
        np.array([[0.1, 0.5, 0.4], [0.4, 0.1, 0.5], [0.5, 0.4, 0.1]]),
    )

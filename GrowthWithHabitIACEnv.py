...
from FunctionApprox import ChebyshevApprox2D, ExpLogLinearApprox
...
class GrowthWithHabitIACEnv:
    def __init__(self, params: Optional[Dict[str, float]] = None) -> None:
        ...
        self.c_grids: Optional[np.ndarray] = None

    def compute_ss(self, k: float) -> Tuple[float, float, float, float]:
        i = self.delta * k
    ...
    def foc_residual(self, k: float) -> float:
        i, c, lamda, mu = self.compute_ss(k)
        return mu * (1 / self.beta - 1 + self.delta) - lamda * self.alpha * k ** (
            self.alpha - 1
        )

    def solve_steady_state(self, x0: float = 1, x1: float = 2) -> None:
    ...
    def pea(
        ...
        if init_weight is None:
            k_grids = np.linspace(
                self.k_ss - k_width, self.k_ss + k_width, 50, endpoint=True
            )
        ...
        while not (np.allclose(old_weight, mu_func.get_weights(), atol=atol)):
            ...
            a_series = AR1Process(self.rho, err_params={"sigma": self.sigma}).simulate(
                simulation + 2, 0
            )
            ...
            for t in range(1, simulation + 2):
                ...
                if k_series[t] > 1 / (self.phi * (1 - self.delta)):
                    ...
                    c = min(
                        c,
                        y
                        + k_series[t] * (1 - self.delta)
                        - 0.5 * self.phi * (1 - self.delta) ** 2 * k_series[t] ** 2
                        - 0.5,
                    )
                if iri:
                    c = min(c, y)
            ...
            for t in range(simulation):
                r_prime = (
                    self.alpha
                    * np.exp(a_series[t + 1])
                    * k_series[t + 1] ** (self.alpha - 1)
                )
                mic = 1 + self.phi * i_series[t]  # marginal investment cost
                mic_prime = 1 + self.phi * i_series[t + 1]
                lamda_prime = (
                    mu_series[t + 1] - self.beta * self.b * mu_series[t + 2]
                )  # shadow price of output
                mu_series[t] = self.beta * (
                    self.b * mu_series[t + 1]
                    + (r_prime + (1 - self.delta) * mic_prime) * lamda_prime / mic
                    - (1 - self.delta) * eta_series[t + 1]
                )
            ...
            state_variables = np.stack(
                [
                    k_series[buried + 1 : simulation],
                    np.exp(a_series[buried + 1 : simulation]),
                ]
            ).transpose()
            _, r2 = mu_func.approx(
                state_variables, mu_series[buried + 1 : simulation], lr=lr
            )
        ...
        plt.scatter(
            k_series[buried : simulation - 1], k_series[buried + 1 : simulation]
        )
        k_s = np.linspace(
            np.min(k_series[buried : simulation - 1]),
            np.max(k_series[buried : simulation - 1]),
            100,
        )
    ...
    def utility(self, c, c_last):
        u = np.zeros_like(c)
        u[c > self.b * c_last] = (
            (c - self.b * c_last) ** (1 - self.gamma) / (1 - self.gamma)
            if self.gamma != 1
    ...
    def value_func_approx(self, k_width, c_width, n_k, n_c, atol=1e-5):
        ...
        def update_value(i, j):
            ...
            invests = invests[
                y - invests - self.phi * invests * invests / 2 <= self.c_ss + c_width
            ]
            invests = invests[
                y - invests - self.phi * invests * invests / 2 >= self.c_ss - c_width
            ]
            invests = invests[
                y - invests - self.phi * invests * invests / 2 > c_last * self.b
            ]
            ...
            for idx in range(len(v_primes)):
                ...
                if not (
                    -k_width <= k_primes[idx] - self.k_ss <= k_width
                    and -c_width <= cs[idx] - self.c_ss <= c_width
                ):
        ...
        while not np.allclose(value_func.weights, old_weights, atol=atol):
            ...
            Parallel(n_jobs=8)(
                delayed(update_value)(i, j)
                for i, j in product(range(len(k_grids)), range(len(c_grids)))
            )
            """
    ...
    def plot_policy(self):
        ...
        for c in range(len(self.c_grids)):
            plt.plot(
                self.k_grids,
                self.policy_func[:, c],
                label=str(round(self.c_grids[c], 1)),
            )

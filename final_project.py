...
if __name__ == "__main__":
    ...
    burned = 1000
    agg_k, agg_y, agg_c, agg_i, income_vec, kvec, idxz, z = env.full_simulation(
        length=simulation_length
    )

    # problem f
    rbc_mean, rbc_cov, rbc_corr, rbc_ar = gen_time_series_moments(
        np.array([agg_c, agg_i, agg_k, agg_y])[:, burned:]
    )
    logger.info(
        "Business cycle data mean: consumption {}, investment {}, capital stock {}, output {}.".format(
            rbc_mean[0], rbc_mean[1], rbc_mean[2], rbc_mean[3]
        )
    )
    logger.info(
        "Business cycle data covariance matrix (c, i, k, y):\n {}.".format(rbc_cov)
    )
    logger.info(
        "Business cycle data correlation matrix (c, i, k, y):\n {}.".format(rbc_corr)
    )
    logger.info(
        "Business cycle data auto-correlation: consumption {}, investment {}, capital stock {}, output {}.".format(
            rbc_ar[0], rbc_ar[1], rbc_ar[2], rbc_ar[3]
        )
    )

    logger.info(
        "Normalized business cycle data mean (Ey = 1): consumption {}, investment {}, capital stock {}, output {}.".format(
            rbc_mean[0] / rbc_mean[3],
            rbc_mean[1] / rbc_mean[3],
            rbc_mean[2] / rbc_mean[3],
            1,
        )
    )
    logger.info(
        "Normalized business cycle data covariance matrix (c, i, k, y):\n {}.".format(
            rbc_cov / rbc_mean[3] ** 2
        )
    )
    ...
    logger.info(
        "Forecasting: real series mean {}, real series median {}.".format(
            np.mean(real_k), np.median(real_k)
        )
    )
    forcast_ks = []
    for p in periods:
        ...
        logger.info(
            "Forecast over {} period(s): average forecast error {}, forecast mean {}, forecast median {}.".format(
                p,
                np.mean(np.abs(forcast_k - real_k)),
                np.mean(forcast_k),
                np.median(forcast_k),
            )
        )
    ...
    for vidx, vvec in enumerate([kvec, income_vec]):
        ...
        logger.info(
            "{} gini index for the last ten periods: pooled gini {}, averaged gini {}, average of period gini {}".format(
                v_name[vidx], pooled_gini, average_gini, gini_average
            )
        )
        plt.plot(
            np.linspace(0, 1, len(pooled_lorenz), endpoint=True),
            pooled_lorenz,
            label="Gini pooled",
        )
        plt.plot(
            np.linspace(0, 1, len(average_lorenz), endpoint=True),
            average_lorenz,
            label="Gini averaged",
        )
        plt.plot(
            np.linspace(0, 1, len(lorenz_average), endpoint=True),
            lorenz_average,
            label="average Gini",
        )
        plt.plot(
        ...
        mean, var, skw, kur = (
            ...
            np.var(vvec, axis=1),
            skew(vvec, axis=1),
            kurtosis(vvec, axis=1),
        )
        for name, v in zip(
            ["mean", "variance", "skewness", "kurtosis"], [mean, var, skw, kur]
        ):
            plt.plot(v[-1000:])
            plt.title("{} of {} distribution".format(name, v_name[vidx]))
            plt.savefig(
                os.path.join(figure_path, "{}_{}.png".format(name, v_name[vidx]))
            )
            plt.savefig(
                os.path.join(figure_path, "{}_{}.pdf".format(name, v_name[vidx]))
            )
            plt.show()

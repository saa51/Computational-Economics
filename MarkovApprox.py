...
class MarkovApprox:
    def __init__(self, rho: float, var_e: float, mean_z: float) -> None:
        self.rho = rho
        self.var_e = var_e
        self.mean_z = mean_z
...
class Rowenhorst(MarkovApprox):
    def approx(self, n: int) -> Tuple[np.ndarray, np.ndarray]:
        p = (1 + self.rho) / 2
        ...
        for i in range(3, n + 1):
            ...
            trans_mat = p * (upper_left + lower_right) + (1 - p) * (
...
class Tauchen(MarkovApprox):
    _omega_dict: Dict[int, float] = {5: 1.6425, 25: 2.5107, 10: 1.9847}

    def approx(self, n: int, m: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:

        if m is None:
            m = Tauchen._omega_dict.get(n, 3)
        grids = np.linspace(
            self.mean_z - m * self.sigma_z,
            self.mean_z + m * self.sigma_z,
            n,
            endpoint=True,
        )
    ...
    def approx(self, n: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(n):
            for j in range(n):
                ...
                prob_z = norm.pdf(
                    (grids[j] - (1 - self.rho) * self.mean_z - self.rho * grids[i])
                    / self.sigma_e
                )
...
def markov_moments(grids: np.ndarray, trans_mat: np.ndarray) -> List[float]:
    ...
    eps = -rho * grids.reshape((-1, 1)) + grids.reshape((1, -1))
    sigma_e = np.sqrt(np.sum(eps * eps * (trans_mat.transpose() * dist).transpose()))

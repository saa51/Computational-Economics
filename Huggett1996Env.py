...
class Huggett1996Env:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.GY_ratio = kwargs.get(
            "GY_ratio", Huggett1996Env._default_params["GY_ratio"]
        )
        ...
        (
            self.value_mat,
            self.consumption_mat,
            self.total_income_mat,
            self.labor_income_mat,
        ) = (None, None, None, None)
    ...
    def solve_decision_rule(self, amax, anum, w, r, b, tau, a_adj=0.1):
        ...
        # init final period
        total_income_mat[-1] = (
            np.kron(a_grids.reshape((-1, 1)), np.ones((1, self.znum))) * (r * (1 - tau))
            + b
        )
        consumption_mat[-1] = (
            np.kron(a_grids.reshape((-1, 1)), np.ones((1, self.znum)))
            * (1 + r * (1 - tau))
            + b
        )
        value_mat[-1] = np.kron(
            self.utility(a_grids * (1 + r * (1 - tau)) + b), np.ones(self.znum)
        ).reshape((anum, self.znum))
        ...
        for t in range(self.N - 2, -1, -1):
            ...
            labor_income_mat[t] = (
                ...
                * w
                * self.yvec[t]
                * np.kron(self.zvec.reshape((1, -1)), np.ones((anum, 1)))
            )
            total_income_mat[t] = (
                a_grids.reshape((-1, 1)) * r * (1 - tau) + labor_income_mat[t] + b_t
            )  # a, z
            cash_on_hand = (
                a_grids * (1 - self.delta) + total_income_mat[t].T
            ).T  # a, z
            future_value = np.dot(value_mat[t + 1], self.pimat.transpose())  # a', z
            c_mat = (
                np.tile(np.expand_dims(cash_on_hand, axis=-1), (1, 1, anum)) - a_grids
            )
            ...
            consumption_mat[t] = np.take_along_axis(
                c_mat, np.expand_dims(policy_mat[t], axis=-1), axis=-1
            ).squeeze(axis=-1)
    ...
    def gen_stationary_distribution(self, avec, policy_mat):
        ...
        for t in range(1, self.N):
            ...
            for idxa, idxz in product(range(anum), range(self.znum)):
                dists[t][policy_mat[t - 1][idxa][idxz]] += (
                    dists[t - 1][idxa][idxz] * self.pimat[idxz]
                )
        return dists

    def solve_stationary_equilibrium(
        self, amax, anum, r_init=-0.00016, max_iter=500, log=False
    ):
        # cohort distribution
        ...
        for iter in range(max_iter):
            ...
            r_supply = (
                self.A * self.alpha * (self.agg_labor / agg_a) ** (1 - self.alpha)
                - self.delta
            )

            if log:
                print(
                    f"Time {time.time() - start_time:.4f}, Iter {iter}: demand r {r:.4f}, supply r {r_supply:.4f}.\nAggregate capital {agg_k:.4f}, aggregate savings {agg_a:.4f}."
                )
                print(
                    f"output {y:.4f}, wage {w:.4f}, transfer {b:.4f}, tax rate {tau:.4f}, aggregate labor {self.agg_labor:.4f}."
                )
    ...
    def gini_index(self):
        ...
        for a in range(self.anum - 1):
            gini += (a_cdf[a + 1] - a_cum[a + 1] + a_cdf[a] - a_cum[a]) * (
                a_cdf[a + 1] - a_cdf[a]
            )
        return gini
    ...
    def plot_variance(self, figure_path=None):
        ...
        ti_var = np.sum(
            (np.log(ti_mat) - ti_mean.reshape((-1, 1))) ** 2 * dists, axis=1
        )
        ...
        li_var = np.sum(
            (np.log(li_mat) - li_mean.reshape((-1, 1))) ** 2 * dists, axis=1
        )

...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-not_all", action="store_true", help="whether run all problems"
    )
    ...
    if not args.not_all or args.vfi:
        ...
        sog.plot_capital_diff(
            title="C_VFI", fname=os.path.join(figure_path, "C_KDiff.png")
        )
        start_time = time.time()
        ...
        # Problem d:
        sog.plot_value(
            title="D_VFI", fname=os.path.join(figure_path, "D_ValueFunc.png")
        )
        sog.plot_value_derivative(
            title="D_VFI", fname=os.path.join(figure_path, "D_ValueFuncD.png")
        )
        sog.plot_value_2derivative(
            title="D_VFI", fname=os.path.join(figure_path, "D_ValueFunc2D.png")
        )

        # Problem e:
        sog.plot_policy(
            title="E_VFI", fname=os.path.join(figure_path, "E_PolicyFunc.png")
        )
        sog.plot_policy_derivative(
            title="E_VFI", fname=os.path.join(figure_path, "E_PolicyFuncD.png")
        )
        sog.plot_policy_2derivative(
            title="E_VFI", fname=os.path.join(figure_path, "E_PolicyFunc2D.png")
        )
        sog.plot_capital_diff(
            title="E_VFI", fname=os.path.join(figure_path, "E_KDiff.png")
        )

        # Problem f:
        errs = sog.plot_euler_err(
            title="F_VFI", fname=os.path.join(figure_path, "F_EulerErr.png")
        )
        titles = ["mean", "min", "max"]
        index = [str(round(a, 2)) for a in sog.a_grids]
        content = [
            [
                round(float(np.mean(err)), 3),
                round(np.min(err), 3),
                round(np.max(err), 3),
            ]
            for err in errs
        ...
        with open(os.path.join(tex_path, "ps2_f.tex"), "w+") as f:
            print(
                write_latex_table(content, title=titles, index=index, align="c"), file=f
            )
        ...
        print(
            "Are all k_t != k_max or k_min?",
            np.all(
                np.logical_and(
                    np.min(sog.k_grids) + 1e-6 < k, k < np.max(sog.k_grids) - 1e-6
                )
            ),
        )
    ...
    if args.hpi or not args.not_all:
        optimize_dict = {"methods": ["HPI"], "HPI_policy_iter": 5, "HPI_value_iter": 20}
        start_time = time.time()
        sog.value_func_iter(400, 5, sog.k_ss * width, 3, **optimize_dict)
        end_time = time.time()
        print(
            "Problem h: Howard Policy Iteration, nh = 5, nv = 20, time =",
            end_time - start_time,
        )
        ...
        end_time = time.time()
        print(
            "Problem h: Howard Policy Iteration, nh = 20, nv = 10, time =",
            end_time - start_time,
        )
        sog.plot_policy(
            title="H_HPI", fname=os.path.join(figure_path, "H_PolicyFunc.png")
        )

    # Problem i:
    if args.mqp or not args.not_all:
        optimize_dict = {"methods": ["MQP"]}
        start_time = time.time()
        sog.value_func_iter(400, 5, sog.k_ss * width, 3, **optimize_dict)
        end_time = time.time()
    ...
    if args.che or not args.not_all:
        ...
        sog.plot_value(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_ValueFunc.png")
        )
        sog.plot_value_derivative(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_ValueFuncD.png")
        )
        sog.plot_value_2derivative(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_ValueFunc2D.png")
        )
        sog.plot_policy(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_PolicyFunc.png")
        )
        sog.plot_policy_derivative(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_PolicyFuncD.png")
        )
        sog.plot_policy_2derivative(
            title="J_Chebyshev", fname=os.path.join(figure_path, "J_PolicyFunc2D.png")
        )

        # Problem k
        errs = sog.plot_euler_err(
            title="K_Chebyshev", fname=os.path.join(figure_path, "K_EulerErr.png")
        )
        titles = ["mean", "min", "max"]
        index = [str(round(a, 2)) for a in sog.a_grids]
        content = [
    ...
    if not args.not_all:
        ...
        with open(os.path.join(tex_path, "ps2_n.tex"), "w+") as f:
            print(
                write_latex_table(moments, title=titles, index=index, align="c"), file=f
            )
